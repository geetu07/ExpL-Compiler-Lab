...
int genSelf() {
    if (!currClass) {
        printf("genSelf: not inside a class method\n");
        exit(1);
    }
    Memberfunclist *mfl = currClass->Vfuncptr;
    int n = 0;
    while (mfl) {
        if (mfl->Flabel == currMethodLabel) {
            n = lnumParam(mfl->paramlist);
            break;
        }
        mfl = mfl->Next;
    }
...
int genClassMethodCall(tnode *root) {
    ...
    if (base->nodetype == Nself) {
        Memberfunclist *cur = currClass ? currClass->Vfuncptr : NULL;
        int curN = 0;
        while (cur) {
            if (cur->Flabel == currMethodLabel) {
                curN = lnumParam(cur->paramlist);
                break;
            }
            cur = cur->Next;
        }
