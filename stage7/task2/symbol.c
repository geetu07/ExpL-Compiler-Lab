...
struct Classtable *currClassEntry = NULL;
Gsymbol *stkhead=NULL;
TypeTable *currUserType=NULL;
int stop=4096;
int curr=0;
...
Gsymbol* Install(char *name, int type, int size,bool ptr, TypeTable *t, int funclabel){
...
void addToST(tnode*root){
    ...
    switch(root->nodetype){
        ...
        case Ntype: {
            curr = root->type;
            if(root->type == USER_TYPE){
                currUserType   = root->typeTableEntry;
                currClassEntry = NULL;
            }
            else if(root->type == CLASS_TYPE){
                currUserType   = NULL;
                currClassEntry = root->classtableEntry;
            }
            else {
                currUserType   = NULL;
                currClassEntry = NULL;
        ...
        case Nvar: {
            ...
            Gsymbol *x;
            if(curr == CLASS_TYPE){
                x = Install(root->varname, CLASS_TYPE, 1, false, NULL, -1);
                x->classtableEntry = currClassEntry;
            } else {
                x = Install(root->varname, curr, y, false, currUserType, -1);
            }
            root->Gentry = x;
        ...
        case NArr:{
            int dim=0;
            Gsymbol* entry=Install(root->left->varname,curr,getSizeOfArray(root->right,&dim),true,currUserType,-1);
        ...
        case Nderefer:{
            Gsymbol*x=Install(root->left->varname,curr,getSize(root),true,currUserType,-1);
            root->Gentry=x;
            root->type=root->Gentry->type;
            break;
        }
        case NFuncDecl:{
            ...
            Gsymbol *y=Install(id->varname,curr,1,false,currUserType,-1);
            y->func=createNewL();
            addParams(y,p);
        }
    }
    ...
    if(x){
        switch(x->nodetype){
            ...
            case Nvar:{
                ...
                linstall(x->varname,currl,false, userType);
                break;
            }
...
static int getSizeF(FieldList *f){
    FieldList *h=f;
    int x=0;
    while(h){
        x++;
...
static TypeTable *createTypeTableEn(char *name, FieldList *f){
...
TypeTable *createNewType(char *name){
    ...
    TypeTable *t=createTypeTableEn(name,NULL);
    TypeTable *head=tthead;
    if(!head)
        tthead=t;
    else{
        while(head->next)
...
void initializeTypeT(){
     typeTInstall(createTypeTableEn("INT",    NULL));
    typeTInstall(createTypeTableEn("STRING", NULL));
    typeTInstall(createTypeTableEn("BOOL",   NULL));
}

void setFieldsOfType(char *name, struct FieldList *fields) {
    struct TypeTable *type = typeLookup(name);
    if (!type) {
        printf("tupe doesmt exist\n");
        exit(1);
    }

    type->fields = fields;
    type->size = getSizeF(fields);
    findex=0;
}
