...
/* -------------------------------------------------------
 * createClassFieldAccess   base.fieldname
 * ------------------------------------------------------- */
tnode *createClassFieldAccess(tnode *base, char *fname) {
    /* The base must be of CLASS_TYPE */
    ...
    /* Inside a class method, only self.field is allowed (scope rule 2) */
    ...
    /* Build a string-name node for the field name */
...
/* -------------------------------------------------------
 * createClassMethodCall   base.method(args)
 * ------------------------------------------------------- */
tnode *createClassMethodCall(tnode *base, char *mname, tnode *args) {
    ...
    /* Argument type checking (reuses the existing Param-list check) */
    ...
    /* Build the call node: left=base, right=args; varname carries method name */
...
/* -------------------------------------------------------
 * createClassFieldAssign   self.field = expr;
 * ------------------------------------------------------- */
...
/* -------------------------------------------------------
 * createNewObject   ID = new(ClassName);
 * ------------------------------------------------------- */
tnode *createNewObject(tnode *id, char *classname) {
    ...
    /* The variable must have been declared as this class type */
...
/* -------------------------------------------------------
 * createClassMethodDef  (called at end of each method body)
 * ------------------------------------------------------- */
