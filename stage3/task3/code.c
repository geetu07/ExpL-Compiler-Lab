...
//stage3
void generateCondCode(tnode* node,int label){
...
void generateWhileCode(tnode*node){
    int cond=createNewL();
    int bod=createNewL(),rest=createNewL();
    fprintf(target,"L%d:\n",cond);
    generateCondCode(node->left,rest);
...
void generateIfElseCode(tnode*node){
    int ifk=createNewL(),elsek=createNewL(),rest=createNewL();
    tnode*cond=node->left,*conn=node->right,*ifn=conn->left,*elsen=conn->right;
    generateCondCode(cond,elsek);
    generateCode(ifn);
    fprintf(target,"JMP L%d\n",rest);
    fprintf(target,"L%d:\n",elsek);
    generateCode(elsen);
