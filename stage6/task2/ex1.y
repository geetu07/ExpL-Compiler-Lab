...
program : typedefBl GDeclBlock FdefBlock MainBlock	{
				//$$=createConnect($2,$3);
				//generate($3);
				printf("Done!\n");
				exit(0);
			...
			| typedefBl GDeclBlock MainBlock             { printf("Done!\n"); exit(0); }
			| MainBlock
		;

typedefBl: TYPE_start typedefList TYPE_end	{printType();}
		| TYPE_start TYPE_end
		;
...
typedef: ID {createNewType($1->varname);} LBRACE typefieldL RBRACE	{setFieldsOfType($1->varname,$4);}
		;

typefieldL: typefieldL typefield	{$$=mergerField($1,$2);}
		| typefield	{$$=$1;}
		;

typefield: INTT ID SEMICOLON	{$$=createTypeFieldE($2->varname,typeLookup("INT"));}
...
allocStmt : ID ASSIGN ALLOC '(' ')' SEMICOLON                 { $$ = createAlloc($1); }
    | memberAccess ASSIGN ALLOC '(' ')' SEMICOLON                 { $$ = createAlloc($1); }
    ;

freeStmt : FREE '(' ID ')' SEMICOLON                         { $$ = createFree($3); }
    ;
